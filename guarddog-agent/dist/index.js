...
export class GuardDogAgent {
    ...
    startTime;
    constructor() {
        this.blockchain = new BlockchainService(process.env.BSC_RPC_URL || 'https://data-seed-prebsc-1-s1.binance.org:8545', process.env.GUARDIAN_PRIVATE_KEY || '', process.env.NETWORK || 'bscTestnet');
        this.monitor = new WalletMonitor(this.blockchain, parseInt(process.env.THREAT_THRESHOLD || '75'), process.env.MAX_PROTECTION_AMOUNT || '1000');
        this.messaging = new OpenClawMessaging(process.env.OPENCLAW_GATEWAY_URL || 'http://127.0.0.1:18789', process.env.OPENCLAW_GATEWAY_TOKEN || '', {
        ...
        this.moltbook = new MoltbookService(process.env.MOLTBOOK_API_URL || 'https://api.moltbook.com', process.env.MOLTBOOK_API_KEY || '', process.env.MOLTBOOK_SUBMOLT || 'lablab', process.env.MOLTBOOK_ENABLED === 'true');
        ...
        try {
            ...
            for (const [walletAddress, detections] of threats) {
                ...
                for (const detection of detections) {
                    ...
                    await this.moltbook.postThreatDetection(detection.walletAddress, detection.tokenAddress, detection.threatLevel, detection.reason);
                }
                ...
                if (protectableDetections.length > 0) {
                    ...
                    if (txHash) {
                        ...
                        await this.moltbook.postProtectionExecution(walletAddress, protectableDetections[0].tokenAddress, amountStr, txHash);
                    }
                }
            }
    ...
    async start() {
        ...
        setInterval(async () => {
            ...
            await this.moltbook.postSystemStatus(stats.totalWallets, `${dbStats.totalProtections} protections recorded`, this.getUptime());
        }, 6 * 60 * 60 * 1000);
    }
    stop() {
        if (!this.isRunning)
