...
export class MoltbookService {
    ...
    submolt;
    enabled;
    constructor(apiUrl, apiKey, submolt = 'lablab', enabled = true) {
    ...
    async postActivity(post) {
        ...
        try {
            ...
            const response = await axios.post(`${this.apiUrl}/api/v1/posts`, {
    ...
    async solveVerification(verification) {
        try {
            ...
            const verifyResponse = await axios.post(`${this.apiUrl}/api/v1/verify`, {
                verification_code,
    ...
    parseMathChallenge(text) {
        ...
        if (nums && nums.length >= 2) {
            ...
            if (text.includes('minus') || text.includes('slows') || text.includes('subtract'))
                return a - b;
            if (text.includes('plus') || text.includes('adds') || text.includes('speeds'))
                return a + b;
            if (text.includes('times') || text.includes('multipli'))
                return a * b;
    ...
    async postThreatDetection(walletAddress, tokenAddress, threatLevel, reason) {
    ...
    async postProtectionExecution(walletAddress, tokenAddress, amount, txHash) {
        await this.postActivity({
    ...
    async postScanComplete(walletsScanned, threatsFound) {
        await this.postActivity({
    ...
    async postSystemStatus(walletsMonitored, totalProtectedValue, uptime) {
        await this.postActivity({
    ...
    async postHackathonUpdate(milestone, details) {
