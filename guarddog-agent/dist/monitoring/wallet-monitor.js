...
export class WalletMonitor {
    ...
    threatThreshold;
    maxProtectionAmount;
    constructor(blockchain, threatThreshold = 75, maxProtectionAmount = '1000') {
    ...
    async executeProtection(detection) {
        if (!detection.shouldProtect) {
            console.log(`⏭️  Skipping protection (balance exceeds max protection amount)`);
            return null;
        }
        try {
            const txHash = await this.blockchain.protectTokens(detection.walletAddress, detection.tokenAddress, detection.balance, detection.threatLevel, detection.reason);
            return txHash;
        }
        catch (error) {
    ...
    async batchExecuteProtection(walletAddress, detections) {
